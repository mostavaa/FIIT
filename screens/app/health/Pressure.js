    ...
    getData.then(resp => {
      if (resp.success) {
        ...
        setChartData({
          ...
          datasets: [
            {
              data: resp.payload[0].map(diastolic =>
                parseFloat(diastolic.value),
              ),
            },
            {
              data: resp.payload[1].map(systolic => parseFloat(systolic.value)),
            },
          ],
        });
      }
                ...
                <View style={{marginTop: 30}}>
                  ...
                  <View
                    ...
                    <View>
                      ...
                      <Text>{new Date(response[1].date).toDateString()}</Text>
                    </View>
                    <Text
                      style={{
                        color: theme.colors.primary,
                        ...
                        {new Date(
                          response[
                            response.findIndex(el => {
                              return (
                                parseInt(el.value) ===
                                Math.max.apply(0, chartData.datasets[1].data)
                              );
                            })
                          ].date,
                        ).toDateString()}
                        ...
                        {new Date(
                          response[
                            response.findIndex(el => {
                              return (
                                parseFloat(el.value) ===
                                Math.min.apply(0, chartData.datasets[1].data)
                              );
                            })
                          ].date,
                        ).toDateString()}
